...
layui.define(['jquery', 'form', 'layer', 'element'], function(exports) {
	 ...
	 * 如果有子级就展开，没有就打开frame
	 */
	$('.layui-layout #nav li').click(function(event){
		alert('ggggggggg')
		if ($(this).children('.sub-menu').length) {
			...
			if ($(this).hasClass('open')) {
				...
				alert('xxxxx')
				$(this).addClass('open');
				$(this).children('a').find('.layui-icon').html('&#xe6a6;');
				$(this).children('.sub-menu').stop().slideDown();
				$(this).siblings().children('.sub-menu').stop().slideUp();
				$(this).siblings().find('.layui-icon').html('&#xe697;');
			...
			var index = $('.layui-side #layui-nav li').index($(this));
